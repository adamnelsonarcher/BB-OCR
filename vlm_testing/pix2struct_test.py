...
"""
Pix2Struct test script for VLM testing.
This script tests Google's Pix2Struct model on book cover images.
"""
...
def run_inference(image_path, prompts):
    """
    Run inference on an image using Pix2Struct model.
    ...
    # Initialize the model - using Pix2Struct which is good at extracting text from images
    print("Loading Pix2Struct model...")
    model_name = "google/pix2struct-textcaps-large"
    ...
    final_results = {
        ...
        "model_type": "pix2struct-textcaps-large",
        "timing": timing_info,
        "results": results
    }
    ...
    output_file = os.path.join("results/json", f"{base_name}_pix2struct_results.json")
...
def main():
    parser = argparse.ArgumentParser(description="Pix2Struct test for VLM on book cover images")
