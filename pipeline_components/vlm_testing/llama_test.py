...
"""
Llama-3 test script for VLM testing.
This script tests Llama-3 with vision capabilities on book cover images.
"""
...
def run_inference(image_path, prompts):
    """
    Run inference on an image using Llama-3 model.
    ...
    # Initialize the model - using Llama-3 with vision capabilities
    print("Loading Llama-3 model...")
    # Using Meta's latest Llama-3 model with vision capabilities
    model_name = "meta-llama/Llama-3-8b-vision"
    ...
    for prompt in prompts:
        ...
        detailed_prompt = f"Look at this book cover image and answer the following question as accurately as possible. {prompt}"
    ...
    output_file = os.path.join("results/json", f"{base_name}_llama3_results.json")
...
def main():
    parser = argparse.ArgumentParser(description="Llama-3 test for VLM on a single image")
